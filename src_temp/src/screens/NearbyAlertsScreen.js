...
import { doc, getDoc, collection, getDocs, query, orderBy, limit, updateDoc, deleteDoc, Timestamp, onSnapshot } from 'firebase/firestore';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        if (data.read === false) {
          unreadAlertIds.push(docSnap.id);
        }
      });
...
const styles = StyleSheet.create({
  ...
  clearButtonText: { color: COLORS.white, fontSize: FONT_SIZES.sm, fontWeight: '600', marginLeft: 4 },
