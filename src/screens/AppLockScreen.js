...
const AppLockScreen = () => {
    ...
    const checkAppLockPreference = async () => {
        try {
            if (!user) {
                // If definitely not logged in, go to Auth
            ...
            if (userDoc.exists()) {
                ...
                const lockSetting = data.appLockEnabled === true; // Explicitly check if TRUE

                setHasPIN(pinExists);
                setAppLockEnabled(lockSetting && pinExists);

                checkBiometricAvailability(lockSetting && pinExists);

                // CRITICAL: Check local variables, not state (state updates are async)
                if (!lockSetting || !pinExists) {
                    console.log("App lock not required, bypassing...");
                    navigation.replace('MainTabs');
                } else {
                    console.log("App lock is ACTIVE");
                    setLoading(false); // Stop showing general loading if we need the lock screen
                }
            } else {
            ...
            navigation.replace('MainTabs'); // Fallback to main app in case of error
        }
    };

    const checkBiometricAvailability = async (isLockRequired = false) => {
        try {
            ...
            if (hasHardware && isEnrolled && isLockRequired) {
