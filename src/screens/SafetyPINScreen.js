...
const SafetyPINScreen = ({ route }) => {
  ...
  const [setupPin, setSetupPin] = useState(''); // Store first PIN for confirmation
  ...
  const handlePINSubmit = async (enteredPin) => {
    ...
    try {
      if (mode === 'verify') {
        ...
        if (step === 1) {
          // First entry - store it and move to confirmation
          setSetupPin(enteredPin);
          setStep(2);
          setPin('');
        } else if (step === 2) {
          // Confirmation - compare enteredPin with the one from step 1 (setupPin)
          if (enteredPin !== setupPin) {
            Alert.alert('PINs do not match', 'The second PIN entered does not match the first one. Please try again.');
            setPin('');
            setSetupPin('');
            setStep(1);
            setLoading(false);
        ...
        {row.map((item, index) => {
          ...
          if (item === 'back') {
            return (
              ...
              >
                <Ionicons name="backspace" size={scale(24)} color={COLORS.white} />
              </TouchableOpacity>
            );
          }
          return (
    ...
    <GradientBackground colors={GRADIENTS.dark}>
      <View style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.canGoBack() ? navigation.goBack() : navigation.replace('MainTabs')}>
        ...
        <View style={styles.content}>
          <View style={styles.iconContainer}>
            <View style={styles.iconCircle}>
              <Ionicons name="lock-closed" size={scale(50)} color={COLORS.white} />
            </View>
          </View>
          ...
          {mode === 'verify' && savedPIN && (
            <View style={styles.verifyActions}>
              ...
              >
                <Ionicons name="create" size={normalize(20)} color={COLORS.primary} />
              ...
              >
                <Ionicons name="trash" size={normalize(20)} color={COLORS.danger} />
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: normalize(24), fontWeight: 'bold', color: COLORS.white, marginBottom: 8, textAlign: 'center' },
  subtitle: { fontSize: normalize(16), color: COLORS.white + '80', marginBottom: SPACING.xl, textAlign: 'center' },
  ...
  numberPad: { width: '100%', maxWidth: scale(300) },
  numberRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: scale(16) },
  numberButton: {
    width: scale(70),
    height: scale(70),
  ...
  numberText: { fontSize: normalize(28), fontWeight: '600', color: COLORS.white },
