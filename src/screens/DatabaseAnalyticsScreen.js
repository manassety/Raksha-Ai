...
const DatabaseAnalyticsScreen = () => {
    ...
    const downloadWeeklyReport = async () => {
        try {
            const reportContent = `RakshaAI - Weekly Database Analysis Report
Generated: ${new Date().toLocaleString()}
--------------------------------------------------

1. USER STATISTICS
- Total Users: ${databaseStats.totalUsers}
- Users Joined This Week: ${databaseStats.usersThisWeek}
- Active Users (Past 7 Days): ${databaseStats.userActivity.reduce((a, b) => a + b.activeUsers, 0)}

2. COMPLAINT & ALERT METRICS
- Total Complaints: ${databaseStats.totalComplaints}
- Total SOS Alerts: ${databaseStats.totalSOS}
- Evidence Files Uploaded: ${databaseStats.totalEvidence}

3. CRIME DISTRIBUTION SUMMARY
${Object.entries(databaseStats.crimeDistribution).map(([type, count]) => `  - ${type}: ${count}`).join('\n')}

4. RECENT ACTIVITY (Last 10 changes)
${databaseStats.recentChanges.map(c => `  - [${c.time}] ${c.action.toUpperCase()} (${c.type}): ${c.details}`).join('\n')}

--------------------------------------------------
End of Report`;
            
            const fileName = `RakshaAI_WeeklyReport_${new Date().toISOString().split('T')[0]}.txt`;
            ...
            if (isAvailable) {
                await Sharing.shareAsync(fileUri, {
                    mimeType: 'text/plain',
                    dialogTitle: 'Download Weekly Analysis Report',
                });
            } else {
            ...
            Alert.alert("Error", "Failed to generate download report");
        }
    };
            ...
            <View style={styles.container}>
                <View style={styles.header}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <TouchableOpacity onPress={downloadWeeklyReport} style={{ marginRight: SPACING.md }}>
