...
const ComplaintDashboardScreen = () => {
    ...
    useEffect(() => {
        if (!user) return;
        const complaintsQuery = (userData?.isAdmin || userData?.isMainAdmin)
        ...
        const unsubscribe = onSnapshot(complaintsQuery, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                allFetchedComplaints.push(data);
            ...
            setGlobalStats({
                total: allFetchedComplaints.length,
            ...
            setComplaints(allFetchedComplaints.sort((a, b) => {
    ...
    const handleConfirmResolution = async (complaintId, agreed) => {
        try {
            if (agreed) {
                ...
                // Remove Evidence related to this user 
                try {
                    const eq = query(collection(db, 'evidence'), where('userId', '==', user.uid));
                    const snap = await getDocs(eq);
                    snap.forEach(async (d) => {
                        await updateDoc(doc(db, 'evidence', d.id), { isDeleted: true });
                    });
                } catch(e) { console.log(e); }
                Alert.alert('Thank You', 'We are glad the issue was resolved to your satisfaction. Evidence has been removed.');
                await updateDoc(doc(db, 'complaints', complaintId), {
                    status: 'in-progress',
                    priority: 'high',
                ...
                Alert.alert('Status Updated', 'The complaint has been reopened with priority status.');
            }
        } catch (error) {
    ...
    const ComplaintItem = ({ item }) => {
        ...
        return (
            <View style={[styles.complaintCard, isMine && styles.myComplaintCard]}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        {isMine && (
                            <View style={styles.myBadge}>
                                <Text style={styles.myBadgeText}>My Report</Text>
                            </View>
                        )}
                    </View>
                ...
                {isMine && item.status === 'resolved' && !item.userConfirmation && (
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <View style={styles.myComplaintsHeader}>
                            {(userData?.isAdmin || userData?.isMainAdmin) ? (
                                <View style={styles.filterContainer}>
                                    ...
                                    >
                                        <Text style={[styles.filterBtnText, filter === 'all' && styles.filterBtnTextActive]}>All Cases</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.filterBtn, filter === 'mine' && styles.filterBtnActive]}
                                        onPress={() => setFilter('mine')}
                                    >
                                        <Text style={[styles.filterBtnText, filter === 'mine' && styles.filterBtnTextActive]}>My Cases</Text>
                                    </TouchableOpacity>
                                </View>
                            ) : (
                                <View style={styles.filterContainer}>
                                    <Text style={[styles.filterBtnText, styles.filterBtnActive, { padding: 8, color: COLORS.primary }]}>My Cases</Text>
                                </View>
                            )}
                        ...
                        {complaints.filter(c => filter === 'all' || (user && c.userId === user.uid)).length === 0 ? (
                            ...
                            complaints
                                .filter(c => filter === 'all' || (user && c.userId === user.uid))
                                .map(complaint => (
